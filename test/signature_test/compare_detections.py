...
zone_detections = 0
file_detections = 0
family_detections_total = dict(zip(set(fam_df['Folder']), [0]*len(set(fam_df['Folder'])))) 
family_detections_page = dict(zip(set(fam_df['Folder']), [0]*len(set(fam_df['Folder'])))) 
family_detections_zone = dict(zip(set(fam_df['Folder']), [0]*len(set(fam_df['Folder'])))) 
family_detections_file = dict(zip(set(fam_df['Folder']), [0]*len(set(fam_df['Folder'])))) 
zone_errors = 0
zone_see_page = 0
for key, det in zone_detections_dict.items():
    ...
    if zone_detections_dict.get(key) is not None:
        ...
        family = fam_dict.get(key)
        if family == None:
            family = "unclassified"

        if page_detections_dict.get(key) != {}:
            page_detections += 1
            family_detections_page[family] += 1


        if zone_detections_dict.get(key) != {}:
            zone_detections += 1
            family_detections_zone[family] += 1

        if file_detections_dict.get(key) is not None and file_detections_dict[key]:
            file_detections += 1
            family_detections_file[family] += 1

        total += 1
        family_detections_total[family] += 1
...
# print(page_detections, file_detections)
for family in family_detections_total.keys():

    print(f"Page {family}: {family_detections_page.get(family)}/{family_detections_total.get(family)}")
    print(f"Zone {family}: {family_detections_zone.get(family)}/{family_detections_total.get(family)}")
    print(f"File {family}: {family_detections_file.get(family)}/{family_detections_total.get(family)}")
