...
with open(f"test_outputs/file_{split_train_test}_plain{suf}.json") as fd:    
    file_detections_dict = json.loads(fd.read())

with open(f"test_outputs/zone_{split_train_test}_{pack}{suf}.json") as fd:    
    zone_detections_dict = json.loads(fd.read())

with open(f"test_outputs/page_{split_train_test}_{pack}{suf}.json") as fd:    
...
for key, det in page_detections_dict.items():
    ...
    if zone_detections_dict.get(key) is not None:
        ...
        if zone_detections_dict[key]:
            zone_detections += 1
            if family_detections_zone.get(family) is None:
                family_detections_zone[fam_dict[key]] = 1
            else:
                family_detections_zone[fam_dict[key]] += 1
